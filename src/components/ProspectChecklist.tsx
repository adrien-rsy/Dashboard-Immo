...
import { Plus, Trash2, ChevronDown, CheckSquare, Square, ListChecks } from 'lucide-react';
...
const ProspectChecklist: React.FC<ProspectChecklistProps> = ({ items, onChange }) => {
  ...
  const [templates] = useState<ChecklistTemplate[]>(() => {
    try {
      const saved = localStorage.getItem('immo_checklist_templates');
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            ...
            {showTemplates && (
              <div className="absolute right-0 top-full mt-1 w-52 bg-white rounded-2xl shadow-xl border border-gray-100 z-50 overflow-hidden">
                {templates.map(tpl => (
                  <button
                    key={tpl.id}
                    type="button"
      ...
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 px-4 py-2.5 bg-gray-50 border border-gray-100 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-black transition-all placeholder:text-gray-300"
        />
        <button
          type="button"
          onClick={addItem}
