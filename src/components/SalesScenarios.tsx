...
const SalesScenarios = ({ caBase, costTotal }: { caBase: number, costTotal: number }) => {
  const scenarios = [
    {
      title: 'Pessimiste',
      icon: TrendingDown,
  ...
  return (
    <div className="bg-white p-8 rounded-[2.5rem] shadow-sm h-full">
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-500 mt-1">Analyse comparative basée sur le CA actuel</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {scenarios.map((s) => {
          const margin = s.price - costTotal;
          const percent = s.price > 0 ? (margin / s.price) * 100 : 0;
            ...
            >
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-bold">{formatEuro(s.price)}</span>
                </div>
                <div className="flex justify-between items-center">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className={cn("text-sm font-bold", s.color)}>{percent.toFixed(1)}%</span>
                </div>
