  ...
  return (
    <div className="bg-white p-8 rounded-[2.5rem] shadow-sm w-full">
      ...
      <Dialog open={!!editingScenario} onOpenChange={(open) => { if(!open) { setEditingScenario(null); setEditForm(null); } }}>
        <DialogContent className="sm:max-w-[850px] rounded-2xl sm:rounded-[2.5rem] h-auto sm:h-[90vh] flex flex-col p-0 overflow-hidden border-none shadow-2xl w-[calc(100%-2rem)] sm:w-auto max-h-[90vh]" onOpenAutoFocus={(e) => e.preventDefault()}>
          {editForm ? (
            <>
      ...
      {innerEditingCost && (
        <Dialog open={!!innerEditingCost} onOpenChange={(open) => !open && setInnerEditingCost(null)}>
          <DialogContent className="sm:max-w-[425px] rounded-2xl sm:rounded-[2rem] z-[100] w-[calc(100%-2rem)] sm:w-auto">
            <DialogHeader>
