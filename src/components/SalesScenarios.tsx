...
const SalesScenarios = ({ scenarios, lots, costs, onUpdate, onDeleteScenario, onSetDefault, onAddScenario, onDuplicateScenario, onAddSpecificCost, calculateTotals }: any) => {
  ...
  const handleCreateGroup = () => {
    ...
    const group = { id: `group_${Date.now()}`, lotIds: newGroup.lotIds, price: Number(newGroup.price) };
    setEditForm({ ...editForm, groupedSales: [...editForm.groupedSales, group] });
    setNewGroup({ lotIds: [], price: '' });
    setShowAddGroup(false);
  };

  const removeGroup = (groupId: string) => {
    setEditForm({ ...editForm, groupedSales: editForm.groupedSales.filter((g: any) => g.id !== groupId) });
  };
  ...
  const handleInnerCostUpdate = (e: React.FormEvent) => {
    ...
    if (innerEditingCost.type === 'finance') {
      ...
      setEditForm({ ...editForm, metadata: { ...editForm.metadata, isNotaireReduced: innerEditingCost.isNotaireReduced }, costNotes: { ...(editForm.costNotes || {}), [innerEditingCost.id]: innerEditingCost.note } });
    } else {
      setEditForm({ ...editForm, costValues: { ...editForm.costValues, [innerEditingCost.id]: innerEditingCost.currentValue }, costNotes: { ...(editForm.costNotes || {}), [innerEditingCost.id]: innerEditingCost.note } });
    }
  ...
  const calculateLiveFinanced = () => {
    ...
    return Math.max(0, acq + travaux + others + notaire + agence - (innerEditingCost.apport || 0));
  };
  ...
  const computeTotalCosts = (form: any) => {
    ...
    return costs
      ...
      .reduce((acc: number, cost: any) => {
        if (cost.type === 'finance') return acc;
  ...
  return (
    <div className="bg-white p-8 rounded-[2.5rem] shadow-sm w-full">
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={onAddScenario} className="flex items-center gap-2 px-4 py-2 bg-black text-white rounded-xl text-xs font-bold hover:bg-gray-800 transition-all shadow-lg shadow-black/10">
      ...
      <Dialog open={!!editingScenario} onOpenChange={(open) => { if (!open) { setEditingScenario(null); setEditForm(null); } }}>
        <DialogContent
          className="w-[calc(100vw-2rem)] rounded-2xl p-0 border-none shadow-2xl max-h-[90dvh] overflow-y-auto overscroll-contain sm:max-w-[850px] sm:w-[850px] sm:rounded-[2.5rem] sm:max-h-[90vh] sm:h-[90vh] sm:overflow-hidden sm:flex sm:flex-col"
              ...
              <div className="px-4 sm:px-8 py-5 sm:py-6 space-y-8 sm:space-y-10 w-full min-w-0 sm:flex-1 sm:min-h-0 sm:overflow-y-auto sm:overscroll-contain">
                ...
                <div>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={() => setShowAddGroup(!showAddGroup)} className="flex items-center gap-1 text-[10px] font-bold uppercase text-black hover:bg-gray-50 px-2 py-1 rounded-lg transition-all">
                      <Plus className="w-3 h-3" />Vente groupée
                    </button>
                  </div>
                  ...
                  {editForm.groupedSales?.length > 0 && (
                    <div className="space-y-2">
                      {editForm.groupedSales.map((group: any, index: number) => {
                        ...
                        return (
                          <div key={group.id} className="flex items-center justify-between gap-4 p-4 bg-white border rounded-2xl" style={{ borderColor: `${groupColor}20`, backgroundColor: `${groupColor}05` }}>
                            <div className="flex items-center gap-3">
                              <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ backgroundColor: `${groupColor}20` }}>
                                <Layers className="w-4 h-4" style={{ color: groupColor }} />
                              </div>
                              <div className="flex flex-col">
                                <span className="text-xs font-bold" style={{ color: groupColor }}>Groupe : {group.lotIds.map(id => lots.find(l => l.id === id)?.name).join(', ')}</span>
                  ...
                  {showAddCost && (
                    <div className="mb-6 p-5 bg-blue-50/50 rounded-2xl space-y-4 border border-blue-100 animate-in fade-in slide-in-from-top-2">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Textarea placeholder="Ajouter une note..." value={newSpecificCost.note} autoFocus={false} onChange={e => setNewSpecificCost({ ...newSpecificCost, note: e.target.value })} className="bg-white border-blue-100 text-sm resize-none h-20" />
                      </div>
                      <button onClick={handleAddSpecific} className="w-full py-2.5 bg-blue-600 text-white text-xs font-bold rounded-xl hover:bg-blue-700 transition-all shadow-md shadow-blue-200">Ajouter au scénario</button>
                    </div>
                  )}
